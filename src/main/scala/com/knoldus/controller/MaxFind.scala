...
import com.knoldus.models.{PostWithComments, User, UserWithPost}

import scala.concurrent.ExecutionContext.Implicits.global
import scala.concurrent.Future


class MaxFind(classListAggregate:ClassListAggregator,parse:Parser) {
  ...
  def userWithMostPost(userUrl: String, postUrl: String): Future[String] = {
    ...
    val temp = classListAggregate.userPostMaker(userUrl, postUrl)
  ...
  def userWithMostComment(postUrl: String, commentUrl: String): Future[String] = {
    ...
    val temp = classListAggregate.postCommentMaker(postUrl, commentUrl)
